...
class MyReports extends HTMLElement {
  ...
  async fetchMyReports(userId) {
    try {
      const response = await fetch(
        `https://pet-finder-1.onrender.com/users/${userId}/pets`
      );
          ...
          ${
            ...
            <div class="pet-card" style="border: 1px solid #ddd; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: transform 0.2s;">
              <div style="position: relative;">
                ...
                <span style="position: absolute; top: 10px; right: 10px; background-color: ${
                  pet.status === "lost" ? "#FF4C4C" : "#4caf50"
              ...
              <div style="padding: 15px;">
                ...
                <p style="color: #666; margin: 5px 0; font-size: 14px;"><strong>📍</strong> ${
                  pet.location.name
                }</p>
                ...
                <div style="display: flex; gap: 10px; margin-top: 15px;">
                  ...
                  <button 
                    ...
                    style="flex: 1; padding: 10px; background-color: ${
                      pet.status === "lost" ? "#4caf50" : "#FF4C4C"
                    }; color: white; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 14px;">
                    ${
                      pet.status === "lost"
                        ? "✅ Marcar encontrado"
                        : "🔴 Marcar perdido"
  ...
  addToggleStatusListeners() {
    this.querySelectorAll(".toggle-status-button").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        if (
          !confirm(
            `¿Estás seguro de que querés marcar esta mascota como ${
              newStatus === "found" ? "encontrada" : "perdida"
            }?`
          )
        ) {
          return;
        ...
        try {
          const response = await fetch(
            `https://pet-finder-1.onrender.com/pet-status/${reportId}`,
            {
              method: "PUT",
          ...
          alert(
            `✅ Estado actualizado a: ${
              newStatus === "found" ? "Encontrado" : "Perdido"
            }`
          );
