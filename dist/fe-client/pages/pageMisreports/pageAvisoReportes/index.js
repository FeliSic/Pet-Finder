  ...
  async fetchReports(userId) {
    try {
      const response = await fetch(
        `https://pet-finder-1.onrender.com/users/${userId}/reports`
      );
      if (!response.ok) {
    ...
    this.innerHTML = `
      <div style="padding: 80px 20px 20px; max-width: 1200px; margin: 0 auto;">
        ...
        <p style="text-align: center; color: #666; margin-bottom: 30px;">
          Recibiste ${reports.length} reporte${
      reports.length > 1 ? "s" : ""
    } de avistajes
        </p>
              ...
              <div style="display: flex; align-items: start; gap: 20px;">
                ...
                <div style="flex: 1;">
                  ...
                  <div style="background-color: #e8f5e9; padding: 15px; border-left: 4px solid #4caf50; margin: 10px 0; border-radius: 4px;">
                    ...
                    <p style="margin: 5px 0;"><strong>Teléfono:</strong> <a href="tel:${
                      report.reporterPhone
                    }" style="color: #5982FF;">${report.reporterPhone}</a></p>
                    <p style="margin: 5px 0;"><strong>Ubicación:</strong> ${
                      report.location
                    }</p>
                  </div>
