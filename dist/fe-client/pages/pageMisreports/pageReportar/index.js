class CreateReport extends HTMLElement {
  constructor() {
    ...
    this.createDropzone = () => {
      const dropzoneElement = this.querySelector("#dropzone");
      const preview = this.querySelector(".pet-picture");
      if (this.myDropzone) {
      ...
      this.myDropzone = new Dropzone(dropzoneElement, {
        ...
        init: function () {
          ...
          this.on("success", (file, response) => {
            ...
            this.elementInstance.uploadedImageUrl = imageUrl;
          });
      ...
      this.myDropzone.elementInstance = this;
    };
    this.addFormListener = () => {
      const form = this.querySelector("#report-form");
      form.addEventListener("submit", async (e) => {
        e.preventDefault();
        const nameInput = this.querySelector(".name");
        const bioInput = this.querySelector(".bio");
        const ubiInput = this.querySelector(".ubi");
        ...
        try {
          const response = await fetch(
            "https://pet-finder-1.onrender.com/post-pets",
            {
              method: "POST",
              headers: {
    ...
    this.addCancelListener = () => {
      ...
      cancelBtn.addEventListener("click", () => {
        if (
          confirm(
            "¿Estás seguro de que querés cancelar? Se perderán los datos ingresados."
          )
        ) {
          window.history.pushState({}, "", "/myReports");
          window.dispatchEvent(new PopStateEvent("popstate"));
