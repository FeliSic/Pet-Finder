  ...
  async render(): Promise<void> {
    ...
    if (form) {
      form.addEventListener("submit", async (e: Event) => {
        ...
        const contraseñaActual = this.querySelector<HTMLInputElement>(".contraseñaActual");
        const contraseñaNueva = this.querySelector<HTMLInputElement>(".contraseñaNueva");
        const confirmarContraseña = this.querySelector<HTMLInputElement>(".confirmContraseña");
        ...
        try {
          ...
          const result: ChangePasswordResponse | ErrorResponse = await response.json();

          if (!response.ok) {
            throw new Error(
              (result as ErrorResponse).error || "Error al cambiar la contraseña"
            );
          }

          console.log((result as ChangePasswordResponse).message);
          ...
          const prevError = form.querySelector<HTMLParagraphElement>(".error-display");
